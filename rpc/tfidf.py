...
class TfidfSearchEngine(Tfidf):
    def __init__(self):
        ...
        self.docs=[]
        self.path=[]
    def TSEtrain(self,docs,path):
        self.docs=[]
        self.path=[]
        for i,d in tqdm(enumerate(docs)):
            tmp=self.mcparser.extract(d,self.stop_pos)
            if(len(tmp.split(" "))>2):
                self.docs.append(tmp)
                self.path.append(path[i])
        print(self.docs)
        self.matrix = self.vectorizer.fit_transform(self.docs).toarray()
        return self.matrix
    def register(self,docs,path):
        self.TSEtrain(docs,path)
    def similardoc(self,doc,size=-1):
        ...
        if(self.matrix.size>0):
            ...
            if(size<0):
                size = len(self.docs)
            return [(self.path[i],self.docs[i]) for i in ranking[:size]]
        else:
