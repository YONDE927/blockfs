...
int sftp::fulldownload(std::string from, std::string dest){
    ...
    if(file == NULL){
	LOG_ERROR("Can't open %s,[code]=%d",p.c_str(),sftp_get_error(m_sftp));
	return -1;
    }
    LOG_DEBUG("Sftp opened %s",p.c_str());
    ofs.open(dest,std::ios_base::out|std::ios_base::binary|std::ios_base::trunc);
    LOG_DEBUG("Local opened %s",dest.c_str());
    for(;;){
	...
	if(nbytes==0){
	    ...
	    LOG_ERROR("Failed to sftp_read of %s,%s",from.c_str(),ssh_get_error(m_sftp));
	    sftp_close(file);
	    return -1;
	}
